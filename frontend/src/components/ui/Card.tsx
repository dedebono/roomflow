...
export const Card = ({ children, className = '', onClick }: CardProps) => {
  return (
    <div
      onClick={onClick}
      className={`glass rounded-xl p-5 shadow-lg border border-slate-800/50 transition-all duration-300 ${
...
export const CardTitle = ({ children, className = '' }: { children: ReactNode; className?: string }) => (
  <h3 className={`text-lg font-bold text-slate-100 tracking-tight ${className}`}>{children}</h3>
);
...
export const CardFooter = ({ children, className = '' }: { children: ReactNode; className?: string }) => (
  <div className={`mt-4 pt-4 border-t border-slate-800/40 flex items-center justify-end gap-2 ${className}`}>{children}</div>
);
