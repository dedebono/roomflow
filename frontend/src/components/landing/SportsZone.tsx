...
export const SportsZone = ({ rooms }: { rooms: Room[] }) => {
  return (
    <section id="sports" className="py-24 bg-[#fefefe]">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {rooms.length === 0 && (
            <p className="text-[#747474] col-span-full text-center">No sports venues available right now.</p>
          )}
          {rooms.map((room, i) => (
            <motion.div
              key={room.id}
              ...
              className="bg-[#fefefe] rounded-3xl overflow-hidden border border-[#cbe2f0] hover:border-amber-500/30 group cursor-pointer"
            >
              <div className="h-48 relative flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-[#143258]/40 to-[#264da1]/20" />
                <span className="relative z-10 text-6xl opacity-30 group-hover:opacity-60 transition-opacity">
                  {getSportIcon(room.name)}
                </span>
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[#143258] to-transparent h-16" />
              </div>
              <div className="p-5 space-y-3">
                <h3 className="text-lg font-bold text-[#143258]">{room.name}</h3>
                <div className="flex items-center gap-4 text-sm">
                  <span className="text-[#22c55e] font-semibold">{room.status === 'ACTIVE' ? 'Available Today' : 'Fully Booked'}</span>
                  <span className="text-[#747474]">|</span>
                  <span className="text-[#143258] font-bold">Rp {(room.price || 0).toLocaleString('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}<span className="text-[#747474] font-normal">/hr</span></span>
                </div>
